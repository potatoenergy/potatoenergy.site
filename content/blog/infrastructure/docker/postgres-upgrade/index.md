...
- [ ] Известно имя контейнера БД (`nextcloud-postgres`) и пользователя (`nextcloud`)
...
> 💡 При наличии сомнений - создать полный бэкап тома перед началом: `tar czf backup-volume.tar.gz /var/lib/docker/volumes/...`
...
> ⚠️ Если пользователь не имеет прав суперпользователя, использовать `-U postgres`.
...
Выбрать **один** из вариантов в зависимости от целевой версии.
...
> 💡 **Рекомендация**: использовать **Вариант Б**. Он соответствует стандарту Debian/Alpine и упрощает будущие `pg_upgrade --link`.
...
> ```
>
> Новый контейнер уже создал роль и БД из `stack.env`. `psql` игнорирует дубли и корректно импортирует таблицы. Можно продолжать.
...
- [pg_dumpall Reference](https://www.postgresql.org/docs/current/app-pg-dumpall.html)
