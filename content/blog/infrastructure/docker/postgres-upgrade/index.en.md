...
Choose **one** option based on target version.
...
> ```
>
> The new container already created the role and DB from `stack.env`. `psql` ignores duplicates and correctly imports tables. Continue.
...
→ Container won't find data at the new path, runs initdb, creates an empty DB.
