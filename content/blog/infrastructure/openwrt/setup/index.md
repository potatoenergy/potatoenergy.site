...
Практическое руководство по настройке OpenWRT для продвинутого домашнего использования. Рассматриваются конфигурации, расширяющие функциональность роутера: фильтрация трафика на уровне DNS, безопасный удалённый доступ через WireGuard, мониторинг метрик и автоматические уведомления.

> 💡 Все чувствительные данные заменены на `<...>`. Следует подставить свои значения.
...
## 🌐 DNS: маршрутизация и фильтрация
...
- Блокировка на уровне `0.0.0.0` - трекеры не резолвятся
...
| Модуль          | Что измеряет                       | Назначение                       |
| --------------- | ---------------------------------- | -------------------------------- |
...
**Обоснование выбора prometheus-node-exporter-lua**:
...
→ Проверить правило для порта 80/443 в зоне 'lan'
→ Попробовать по SSH: ssh root@192.168.1.1

# WireGuard не поднимается
→ Проверить ключи: без пробелов, переносов, в одной строке
→ Убедиться, что порт 51820 свободен: netstat -ulpn | grep 51820
→ Проверить маршрут: `ip route | grep wg0`
...
→ Проверить фаервол: разрешён ли входящий на 9101 из 'lan'
```
