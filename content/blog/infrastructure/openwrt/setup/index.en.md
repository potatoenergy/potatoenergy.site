...
Practical guide to OpenWRT configuration for advanced home use. Covers configurations that extend router functionality: DNS-level traffic filtering, secure remote access via WireGuard, metrics monitoring, and automated notifications.

> 💡 All sensitive values are replaced with `<...>`. Substitute your own values.
...
- `prometheus-node-exporter-lua-*` - lightweight Lua modules for metrics collection with minimal resource usage
- `kmod-wireguard` - kernel-space implementation, faster than userspace
- `https-dns-proxy` - encrypts DNS queries, prevents leaks and censorship
...
- `sleep 10` - allows network initialization before sending
...
**Rationale for prometheus-node-exporter-lua**:
