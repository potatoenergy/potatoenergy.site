...
title: "Flatpak: HTTP 403 Error Loading OpenH264 - Fix"
---

When running apps via Flatpak (Discord, OBS, Firefox, etc.), the following warning may appear:
...
**Cause**: Cisco's server (`ciscobinary.openh264.org`) blocks automated downloads of `libopenh264` due to licensing reasons. Status `403` = "forbidden".
...
If Flatpak is used in user mode (`--user`), commands differ slightly:
...
If Flatpak version doesn't work, allow the app to access system libraries:
...
> ⚠️ This reduces container isolation - use only if confident.
...
→ It's a licensing policy. It cannot be changed, but can be worked around.

# "What if openh264 is specifically needed?"
