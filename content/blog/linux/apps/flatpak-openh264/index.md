...
title: "Flatpak: Ошибка 403 при загрузке OpenH264 - решение"
aliases:
  - /blog/linux/flatpak-openh264/
---

При запуске приложений через Flatpak (Discord, OBS, Firefox и др.) может появиться предупреждение:
...
**Причина**: сервер Cisco (`ciscobinary.openh264.org`) блокирует автоматическую загрузку библиотеки `libopenh264` по лицензионным причинам. Статус `403` = "доступ запрещён".

---

## ✅ Решение: заменить OpenH264 на ffmpeg-full

Вместо проблемного кодека установить полнофункциональный `ffmpeg-full` из репозитория Flathub. Он включает все необходимые кодеки, включая H.264, и не зависит от внешних загрузок.
...
При запросе версии выбрать актуальную (обычно `24.08` или выше):
...
flatpak kill org.discordapp.Discord  # заменить на ваш app-id
flatpak run org.discordapp.Discord

# Или просто перезагрузить систему
```
...
Если Flatpak используется в режиме пользователя (`--user`), команды немного отличаются:
...
> ⚠️ Это снижает изоляцию контейнера - использовать только при уверенности.
...
→ Это лицензионная политика. Изменить её невозможно, но можно обойти.

# "А если нужно использовать именно openh264?"
→ Попробовать скачать библиотеку вручную и положить в `~/.var/app/*/cache/openh264/`, но это нестабильно.
