...
aliases:
  - /blog/linux/steamscope/
---

## 🎮 Назначение

Запуск игр через Steam на Linux требует разных флагов для разных движков, различных переменных окружения для AMD и NVIDIA, а также ручного встраивания инструментов вроде Gamescope и MangoHud в команду запуска.
...
| `--resolution=1280x720` | Задаёт разрешение для Gamescope (по умолчанию - нативное)     | Тестирование, снижение нагрузки                           |
...
Добавлять параметры по одному - так проще найти причину проблем.

### Если игра падает

1. **Отключить `--gamescope`** - самый частый источник конфликтов
2. **Отключить `--gamemode`** - может конфликтовать с системными настройками
3. **Проверить логи**:
   ```bash
...
Если игра требует английскую локаль - переопределить в параметрах запуска:
...
→ Попробовать снизить --resolution вместе с --fsr
...
- Скачивать скрипт только с [официального gist](https://gist.github.com/ponfertato/85fc964664423eae1ac0a7ee91d53db6)
- Проверять хэш при обновлении (опционально)
- Не передавать скрипт с чувствительными данными в аргументах
