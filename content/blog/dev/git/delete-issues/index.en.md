...
Long-running projects accumulate outdated issues: bugs for old versions, irrelevant features, test tickets. Manual deletion takes significant time. The script automatically finds and deletes (or marks) old issues by specified labels.
...
- `split(",")` - allows specifying labels as a single string
...
```bash
# 1. Verify selected issues are actually the ones needed
