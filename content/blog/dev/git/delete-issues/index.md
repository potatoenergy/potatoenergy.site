...
aliases:
  - /blog/git/delete-issues/
---

В долгосрочных проектах накапливаются устаревшие задачи: баги для старых версий, неактуальные фичи, тестовые тикеты. Ручное удаление занимает много времени. Скрипт автоматически находит и удаляет (или помечает) старые задачи по заданным меткам.

> 💡 Скрипт использует **dry-run по умолчанию** - сначала показывает, что будет удалено, без реального удаления.
...
Заменить блок удаления на закрытие:
...
→ Делать резервную копию: gh issue list --json ... > backup.json
```
