...
1. Скачать с [git-scm.com](https://git-scm.com/download/win)
2. Запустить установщик, оставить настройки по умолчанию
3. **Важно**: на шаге "Adjusting your PATH" выбрать `Git from the command line and also from 3rd-party software`
...
# Редактор по умолчанию (выбрать один)
...
```bash
# Создать ключ (нажимать Enter на все вопросы)
...
→ Перезапустить терминал или компьютер

# Ошибка "Permission denied (publickey)"
→ Проверить, что публичный ключ добавлен в профиль GitHub/GitLab
→ Проверить: ssh -T git@github.com

# Конфликт переводов строк (CRLF/LF)
→ Настроить core.autocrlf под свою ОС (см. выше)
