...
> 💡 Использовать WSL2. Почти без накладных расходов, но с полной поддержкой Docker, systemd и всех функций Linux.
...
**Перезагрузить компьютер** после выполнения.
...
# Задать имя пользователя и пароль (не отображается при вводе - это нормально)
...
> ⚠️ Не редактировать Linux-файлы из Windows-приложений напрямую (через `\\wsl$`). Это может повредить метаданные. Использовать Linux-инструменты внутри WSL.
...
# Переключаться между оболочками: Ctrl+Shift+1/2/3...
```
...
# Установить расширение "Remote - WSL" если потребуется
```
...
# Settings → Resources → WSL Integration → включить дистрибутив
...
Файл: `%USERPROFILE%\.wslconfig` (создать, если нет)
...
→ Проверить: "Панель управления" → "Программы" → "Включение компонентов Windows" → Hyper-V Platform
...
→ Хранить проекты внутри Linux-файловой системы: ~/projects, а не в /mnt/c/...
→ Использовать VS Code Remote WSL для редактирования
...
- [Настройка .wslconfig](https://learn.microsoft.com/ru-ru/windows/wsl/wsl-config)
