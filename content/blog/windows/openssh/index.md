...
> 💡 После настройки можно подключаться к Windows-ПК как к обычному Linux-серверу: `ssh user@192.168.1.100`
...
2. "Добавить компонент" → найти "OpenSSH Server" → Установить
...
ssh <пользователь>@<IP_адрес_Windows>

# Пример:
ssh kirill@192.168.1.100
```

> 💡 Первый вход запросит подтверждение отпечатка ключа - ввести `yes`.

---

## Аутентификация по ключам (рекомендуется)

### На клиенте (где подключаемся)
...
→ Проверить логи: Get-WinEvent -LogName "OpenSSH/Operational" -MaxEvents 10

# Порт 22 не слушается
→ Проверить брандмауэр: Get-NetFirewallRule -Name sshd
→ Проверить, что служба запущена: Get-Service sshd

# "Permission denied (publickey,password)"
→ Проверить права на authorized_keys (должен читать только владелец)
→ Убедиться, что публичный ключ вставлен одной строкой, без переносов

# Подключение есть, но нет прав на файлы
→ Проверить права пользователя на папки в Windows
→ Попробовать запустить терминал от имени администратора на клиенте
```
