...
[Scrcpy](https://github.com/Genymobile/scrcpy) (Screen Copy) is a tool for displaying and controlling Android devices from a computer.
...
## ⚠️ Main Issue: "Connection refused"

When attempting to start wireless mode directly with:

```bash
scrcpy --tcpip=192.168.x.x
```

The error occurs: `failed to connect to '192.168.x.x:5555': Connection refused`.

**Reason**: By default, the Android debugging daemon (`adbd`) listens only on the USB interface. Port `5555` is closed until the device is explicitly switched to TCP/IP mode.

---

## 🔧 3 Connection Methods (From Simple to Advanced)
...
2. Ensure USB debugging is enabled (and "Allow" is tapped on the phone screen on first connection).
3. Run the command **without specifying an IP address**:
   ```bash
   scrcpy --tcpip
   ```
4. **What happens**: Scrcpy automatically switches the device to TCP/IP mode, detects its Wi-Fi IP address, and connects.
5. Once connected, **unplug the USB cable**. The mirroring continues over Wi-Fi.
...
### Method 3: Fully Wireless (Android 11+)

If no USB cable is available, but the phone and PC are on the same Wi-Fi network.

1. On phone: **Settings** → **Developer options** → **Wireless debugging** (enable).
2. Tap **"Pair device with pairing code"**. The screen shows an IP address, port, and a 6-digit code.
3. On PC, initiate pairing (use the IP and port from step 2):
   ...
   ```
4. Go back to the "Wireless debugging" menu on the phone. It shows a **different** IP and port for actual connection (not for pairing).
5. Connect:
   ```bash
...
**Symptom**: The command executes, but the connection is dropped immediately.
**Reason**: A system prompt is waiting on the phone screen: _"Allow USB debugging from this computer?"_ with an RSA key fingerprint. Until **"Allow"** is tapped (and ideally "Always allow" is checked), `adbd` rejects the authentication.
...
**Reason**: Mode conflict. The `adb pair` command works **only** with the "Wireless debugging" feature (Android 11+). If `adb tcpip 5555` was previously run, the device is already in classic mode and does not accept the pairing protocol.
...
For maximum comfort, these flags are used when launching:
...
| `--no-audio`        | Disables audio forwarding (if only the screen is needed)    | `scrcpy --no-audio`        |
