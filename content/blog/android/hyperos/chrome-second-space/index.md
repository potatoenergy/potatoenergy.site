...
**Проблема**: после перезагрузки системы Chrome во втором пространстве переустанавливается. `user 999` - это ID клонированного профиля, но HyperOS восстанавливает системные приложения при старте.

### Отключение "Клонирования приложений"

**Недостаток**: удаляет **все** клонированные приложения и их данные. Не подходит, если нужны другие дубликаты (мессенджеры, банки).
...
Если настройка Shizuku нежелательна:
...
**Преимущества**: не требует root, Shizuku или ADB  
**Недостатки**: требуется адаптация к новому браузеру
...
**Ограничение**: после перезагрузки телефона скрипт запускается вручную (или настраивается автозапуск через Tasker).
