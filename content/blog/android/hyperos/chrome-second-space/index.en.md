...
**Problem**: after reboot, Chrome in second space is reinstalled. `user 999` is the cloned profile ID, but HyperOS restores system apps on startup.

### Disable "Dual Apps"

**Downside**: removes **all** cloned apps and their data. Not suitable if other duplicates are needed (messengers, banking apps).
...
If setting up Shizuku is undesirable:
...
**Advantages**: no root, Shizuku, or ADB required  
**Disadvantages**: adaptation to new browser is required

---

### Solution 3: ADB script with auto-launch (advanced)

If Shizuku doesn't work, ADB commands can be automated:
...
**Limitation**: after phone reboot, script must be run manually (or auto-launch set up via Tasker).
