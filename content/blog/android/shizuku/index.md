...
Shizuku - сервис, предоставляющий приложениям доступ к системным API Android **без root-прав**. Работает через ADB (Android Debug Bridge), используя привилегии `shell`.
...
> 💡 Shizuku предоставляет ограниченный доступ к системным функциям. Полный root не требуется.
...
> ⚠️ После перезагрузки телефона Shizuku запускается заново (процесс не сохраняется).
...
### Назначение Shizuku для Obtainium
...
[SAI (Split APKs Installer)](https://github.com/Aefyr/SAI) - установщик split-APK, XAPK, APKS (форматы, не поддерживаемые стандартным установщиком).

### Назначение Shizuku для SAI
...
**Преимущество**: Shizuku запускается автоматически при загрузке.
...
### Действия, доступные приложению с доступом к Shizuku
