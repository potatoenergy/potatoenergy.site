...
Shizuku is a service that provides apps with access to Android system APIs **without root**. It works via ADB (Android Debug Bridge), using `shell` privileges.
...
> 💡 Shizuku provides limited access to system functions. Full root is not required.
...
4. After pairing: "Start" → service launches
...
> ⚠️ After phone reboot, Shizuku must be started again (process does not persist).

---

## 🔧 Example 1: Obtainium + Shizuku

[Obtainium](https://github.com/ImranR98/Obtainium) is an app update manager directly from sources (GitHub, GitLab, F-Droid).

### Purpose of Shizuku for Obtainium
...
[SAI (Split APKs Installer)](https://github.com/Aefyr/SAI) is an installer for split-APK, XAPK, APKS (formats not supported by the default installer).

### Purpose of Shizuku for SAI
...
If rooted - install Shizuku as a system app:
...
**Advantage**: Shizuku starts automatically on boot.
...
### Actions available to an app with Shizuku access
